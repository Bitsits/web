//Tag Version: MSDN 06-11-07
var gTagVer = "MSDN 06-11-07";
var gService = true;
var gTimeZone = -8;
...
function dcsDownload(evt){
	...
	if (evt){
        ...
        var gPath=e.pathname?((e.pathname.indexOf("/")!=0)?"/"+e.pathname:e.pathname):"/";				
				if(f.alt){var gTitle=f.alt;}
				else{if(document.all){var gTitle=e.innerText||e.innerHTML||"";}else{gTitle=e.text||e.innerHTML||"";}}
	...
	if (evt){
		...
		if(f){
			if (f.hostname&&f.href&&f.protocol&&(f.protocol.indexOf("http")!=-1)){
				dcsNavigation(f);
        if((navigator.appVersion.indexOf("MSIE")==-1)&&(f.onclick)){dcsSetVarCap(f);}

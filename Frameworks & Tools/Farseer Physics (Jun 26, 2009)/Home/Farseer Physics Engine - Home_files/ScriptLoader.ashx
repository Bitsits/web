...
function dcsDownload(evt){
	...
	if (evt){
		var e=dcsEvt(evt,"A");
		if (typeof(e.hostname) != 'unknown' && e.hostname){
...
function dcsFormButton(evt){
	...
	if (evt){
		...
		var type=e.type||"";
		if (e&&e.form&&e.form.elements&&type&&((type=="submit")||(type=="image")||(type=="button")||(type=="reset"))||((type=="text")&&((evt.which||evt.keyCode)==13))){
		    var qry="";
...
function $submitForm(hiddenId, causesValidation) {
    ...
    if (isValid) {
        $get(hiddenId).value = 'true';
        $get(hiddenId).form.submit();
    }
    return false;
}
...
CodePlex.EnhancedTextBox = function() {
    // protected fields
    var _id, _ff, _limit, _textarea, _counter, _showCounter, _rowState, _autoEnlarge, _maxAutoEnlarge, _defaultButton, _multiLine, _loadTimer, _keyTimer, _showUnsavedDataAlert, _hiddenField;

    // public members    
    this.initialize = function(textareaID, limit, counterID, showCounter, stateID, autoEnlarge, maxAutoEnlarge, defaultButtonID, showUnsavedDataAlert, hiddenFieldID, hiddenFieldValue) {
        ...
        _showUnsavedDataAlert = showUnsavedDataAlert;
        if (_showUnsavedDataAlert && Boolean.parse(_showUnsavedDataAlert)) {
            if (hiddenFieldID)
                _hiddenField = $get(hiddenFieldID);
            if (hiddenFieldValue && hiddenFieldValue == 'true') {
                window.onbeforeunload = function() { return 'You are navigating away from the page you were editing; Your changes may be lost.'; }
                if (_hiddenField)
                    _hiddenField.value = 'true';
            }
        }
    ...
    function populateTextArea() {
        ...
        var txt;
        if (txt = _textarea.getAttribute('txt')) {
            _textarea.removeAttribute('txt');
            _textarea.value = txt;
            onChange();
        }
    }
    ...
    function onKeyPress_MultiLine(e) {
        ...
        if (_ff && keyCode == 13 && e && e.stopPropagation) e.stopPropagation();
...
if (typeof(Sys) !== "undefined") Sys.Application.notifyScriptLoaded();Type.registerNamespace('CodePlex');

CodePlex.Paging = new function() {
    ...
    function dispose() {
        CodePlex.Paging = _this.constructor;
        _this = _parameterCallbackMethod = null;
    }
...
CodePlex.MultiFileUpload.UploadControlsTemplate = function(namePrefix,iconPathFormat) {
...
CodePlex.MultiFileUpload._fileCount = 1;
...
CodePlex.MultiFileUpload.addFileToTable = function(tableId, namePrefix, iconPathFormat) {
    var table = $get(tableId);
    if (table) {
        if(table.tBodies && table.tBodies.length)table = table.tBodies[0];
        CodePlex.MultiFileUpload._fileCount++;
        var template = new CodePlex.MultiFileUpload.UploadControlsTemplate(namePrefix, iconPathFormat);
...
CodePlex.PromptBox=function(promptText){
    ...
    function blurHandler(e){
        var elt=getElement();
        if(!elt)return;
        if (!(elt.dirty = _dirty = (!!elt.value && elt.value != _promptText))) {
    ...
    function registerEvents(disposing){
        ...
        $handler(elt,'keypress', submitHandler);
...
CodePlex.SortableGrid = new function() {
    var _this, _clientID;
    ...
    this.applySort = function(url) {
        var request = new Sys.Net.WebRequest();
        request.set_url(url);
        request.set_httpVerb("GET");
        request.add_completed(this._applySortCallback);
        request.invoke();
    }
    
    this._applySortCallback = function(sender, args) {
        if (sender.get_responseAvailable()) {
            $get(_clientID).innerHTML = sender.get_responseData();
        }
    }
    
    function dispose() {
        CodePlex.SortableGrid = _this.constructor;
...
var m_editWikiPartialUrl;
var m_previewWikiPartialUrl;
var m_saveWikiPartialUrl;
var m_cancelWikiPartialUrl;
var m_wikiContainerId;
var m_editWikiLinkId;

function editWikiContent() {
    CodePlex.UpdateProgress.showWithCustomText(true, 'Loading...');
    $.post(m_editWikiPartialUrl, { WikiAction: 'Edit', WikiContent: $('#currentWikiText').val() }, function(data) {
        $('#'+m_wikiContainerId).empty().html(data);
        $('#'+m_editWikiLinkId).hide();
        CodePlex.UpdateProgress.hide();
    }, 'html');
}

function previewWikiContent() {
    CodePlex.UpdateProgress.showWithCustomText(true, 'Loading...');
    var wikiContent = $('#inputText').val();
    $('#currentWikiText').val(wikiContent);
    $.post(m_previewWikiPartialUrl, { WikiAction: 'Preview', WikiContent: wikiContent }, function(data) {
        $('#' + m_wikiContainerId).empty().html(data);
        CodePlex.UpdateProgress.hide();
    }, 'html');
}
...
function saveWikiContent() {
    CodePlex.UpdateProgress.showWithCustomText(true, 'Saving...');
    var wikiContent = document.getElementById('inputText') ? $('#inputText').val() : $('#currentWikiText').val();
    $.post(m_saveWikiPartialUrl, { WikiAction: 'Save', WikiContent: wikiContent }, function(data) {
        $('#'+m_wikiContainerId).empty().html(data);
        $('#'+m_editWikiLinkId).show();
        CodePlex.UpdateProgress.hide();
    }, 'html');
}

function cancelWikiContent() {
    CodePlex.UpdateProgress.showWithCustomText(true, 'Loading...');
    $.post(m_cancelWikiPartialUrl, { WikiAction: 'View' }, function(data) {
        $('#'+m_wikiContainerId).empty().html(data);
        $('#'+m_editWikiLinkId).show();
        CodePlex.UpdateProgress.hide();
    }, 'html');

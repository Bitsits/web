(function () {
	...
	function initForm($opener, $holder) {
		...
		$holder.find('form').lald('submit', doSubmit);

		$holder.find('a.close').lald('click', doClose);
	...
	function clearRadios($form) {
		$form.find('input:radio[name=reason]:checked').attr('checked', false);
	}
	...
	//Attach events
	//click 'remove' button
	$('a.remove').lald('click', function (event) {
		var $this = $(this);
		var $holder = $(holderID);
